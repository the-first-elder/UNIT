...
export const LIFI_TOOLS: Tool[] = [
  {
    name: "get-token",
    description:
      "Get token details (address, symbol, decimals, chainId) for a given chain and token symbol. Returns the contract address needed for swap quotes.",
    inputSchema: {
      type: "object",
      properties: {
        chain: {
          type: "string",
          description:
            "Chain ID (e.g. 1 for Ethereum, 42161 for Arbitrum, 10 for Optimism, 8453 for Base, 137 for Polygon, 43114 for Avalanche, 56 for BSC)",
        },
        token: {
          type: "string",
  ...
  },
  {
    name: "get-quote",
    description:
      "Get a swap quote between two tokens on the same chain. Returns transaction request data including to, data, value, and approval address.",
    inputSchema: {
      type: "object",
      properties: {
...
async function handleGetToken(args: Record<string, unknown>) {
...
async function handleGetChains() {
  try {
    const data = await cached("chains", async () => {
      ...
      if (!res.ok) throw new Error(`LI.FI chains API returned ${res.status}`);
      return res.json();
    }) as Record<string, unknown>;
...
export async function callLifiTool(name: string, args: Record<string, unknown>) {
  switch (name) {
    case "get-token":
...
export async function lifiGetToken(chain: string, token: string): Promise<{ address: string; symbol?: string; decimals?: number; chainId?: number } | null> {
  try {
...
export async function lifiGetQuote(
  fromChain: string,
  toChain: string,
  fromToken: string,
  toToken: string,
