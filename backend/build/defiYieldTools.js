...
function riskTier(pool) {
    if (pool.tvlUsd < 100_000)
        return "EXTREME";
    if (pool.apy > 100)
        return pool.tvlUsd > 1_000_000 ? "HIGH" : "EXTREME";
    if (pool.apy > 50)
        return pool.tvlUsd > 10_000_000 ? "HIGH" : "MEDIUM";
    if (pool.stablecoin && pool.apy > 15)
        return "HIGH";
    if (pool.stablecoin)
        return "LOW";
...
export const DEFI_YIELD_TOOLS = [
    {
        name: "get_top_yields",
        description: "Find best yield opportunities for a given chain and asset. Returns APY comparison data including pool details, project name, chain, and risk tier.",
        inputSchema: {
            type: "object",
            properties: {
                chain: {
    ...
    {
        name: "get_pool_risk",
        description: "Get detailed risk analysis for a specific yield pool. Returns TVL, APY breakdown, impermanent loss risk, and overall risk tier.",
        inputSchema: {
            type: "object",
            properties: {
                poolId: {
    ...
    {
        name: "compare_yields",
        description: "Compare yields across multiple chains for a given token. Returns best APY per chain with TVL and protocol details.",
        inputSchema: {
            type: "object",
            properties: {
...
async function handleGetTopYields(args) {
    ...
    const maxRisk = args.maxRisk || "HIGH";
    ...
    const filtered = pools.filter((p) => {
        ...
        if (riskOrder.indexOf(tier) > maxRiskIndex)
            return false;
        if (stablecoinOnly && !p.stablecoin)
            return false;
        return true;
    });
