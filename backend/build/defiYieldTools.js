...
async function fetchProtocols() {
    const now = Date.now();
    if (cachedProtocols && now - lastProtocolFetch < PROTOCOL_CACHE_TTL)
...
function formatPool(p, rank = 0) {
    ...
    return (`${prefix}**${p.project || "?"}/${p.symbol || "?"}** on ${p.chain || "?"} ` +
        `[${risk}]\n` +
...
export const DEFI_YIELD_TOOLS = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                chain: {
                    type: "string",
                    description: 'Filter by chain (e.g. "Ethereum", "Polygon", "Arbitrum"). Empty = all chains.',
                },
                min_tvl: {
                    type: "number",
                ...
                max_risk: {
                    type: "string",
                    description: 'Maximum risk tier: "LOW", "MEDIUM", "HIGH", or "EXTREME".',
    ...
    {
        name: "get_pool_risk",
        description: "Deep risk analysis for a specific DeFi pool. " +
        ...
        inputSchema: {
            ...
            properties: {
                pool_id: {
                    type: "string",
                    description: "DefiLlama pool UUID (e.g. 747c1d2a-c668-4682-b9f9-296708a3dd90).",
                },
                project: {
                    type: "string",
                    description: 'Project name to search (e.g. "aave-v3"). Used if pool_id not provided.',
                },
                symbol: {
                    type: "string",
                    description: 'Token symbol to search (e.g. "USDC"). Used with project.',
                },
            },
        },
    },
    {
        name: "compare_yields",
        description: "Compare yield opportunities for a specific deposit amount. " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                chains: {
                    type: "string",
                    description: 'Comma-separated chains to compare (e.g. "Ethereum,Polygon,Arbitrum"). Empty = all.',
                },
                stablecoins_only: {
                    type: "boolean",
                    description: "Only compare stablecoin pools (default True for safety).",
                },
                min_tvl: {
                    type: "number",
    ...
    {
        name: "get_chains",
        description: "List all available chains with pool counts and top yields.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
async function handleGetTopYields(args) {
    ...
    const filtered = pools.filter((p) => {
        ...
        if (chain && (p.chain || "").toLowerCase() !== chain.toLowerCase())
            return false;
        if (stablecoinsOnly && !p.stablecoin)
            return false;
        if (p.outlier)
            return false;
        const risk = riskTier(p);
        if ((riskOrder[risk] ?? 9) > maxRiskVal)
            return false;
        return true;
    });
    ...
    const lines = [
        ...
        `Filters: TVL ≥ $${minTvl.toLocaleString("en-US")} | Risk ≤ ${maxRisk} | ${stablecoinsOnly ? "Stablecoins only" : "All assets"}`,
    ...
    if (!top.length) {
        lines.push("No pools match your criteria. Try lowering min_tvl or increasing max_risk.");
    }
    return { content: [{ type: "text", text: lines.join("\n") }] };
}
async function handleGetPoolRisk(args) {
    ...
    if (poolId) {
        ...
        const matches = pools.filter((p) => project.toLowerCase().includes((p.project || "").toLowerCase()) ||
            (p.project || "").toLowerCase().includes(project.toLowerCase()));
        const symbolFiltered = symbol
            ? matches.filter((p) => (p.symbol || "").toUpperCase().includes(symbol.toUpperCase()) ||
                symbol.toUpperCase().includes((p.symbol || "").toUpperCase()))
            : matches;
        if (symbolFiltered.length) {
            symbolFiltered.sort((a, b) => (b.tvlUsd || 0) - (a.tvlUsd || 0));
    ...
    const auditStatus = { 0: "No audits", 1: "Partially audited", 2: "Fully audited" }[audits] ?? "Unknown";
...
async function handleCompareYields(args) {
    ...
    for (let i = 0; i < Math.min(filtered.length, 20); i++) {
        ...
        lines.push(`${(i + 1).toString().padStart(3)} ${name.padEnd(35)} ${(p.chain || "?").padEnd(12)} ${apyVal.toFixed(2).padStart(6)}% ${risk.padEnd(8)} $${daily.toFixed(2).padStart(8)} $${monthly.toFixed(2).padStart(8)} $${yearly.toFixed(2).padStart(10)}`);
    }
    if (!filtered.length) {
...
async function handleGetChains(_args) {
    const pools = await fetchPools();
    ...
    const sorted = [...chainStats.entries()]
        ...
        .slice(0, 30);
    const lines = ["# DeFi Chains by TVL", ""];
    for (const [chain, stats] of sorted) {
        lines.push(`- **${chain}**: ${stats.count} pools | TVL: $${stats.totalTvl.toLocaleString("en-US")} | Top yield: ${stats.maxApy.toFixed(1)}% (${stats.maxApyPool})`);
    }
    return { content: [{ type: "text", text: lines.join("\n") }] };
}
export async function callDefiYieldTool(name, args) {
    switch (name) {
        case "get_top_yields":
