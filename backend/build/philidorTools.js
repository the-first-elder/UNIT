...
function riskScore(pool) {
    let s = 0;
    if ((pool.tvlUsd || 0) < 100_000)
        s += 3;
    else if ((pool.tvlUsd || 0) < 1_000_000)
        s += 2;
    else if ((pool.tvlUsd || 0) < 10_000_000)
        s += 1;
    if (pool.ilRisk === "yes")
        s += 2;
...
export const PHILIDOR_TOOLS = [
    {
        name: "search_vaults",
        description: "Search for yield vaults by asset and chain. Returns risk scores, APY, TVL, and protocol details. Risk levels: safe, moderate, risky, dangerous.",
        inputSchema: {
            type: "object",
            properties: {
                asset: {
    ...
    },
    {
        ...
        description: "Get detailed risk breakdown for a specific vault/protocol. Returns TVL analysis, APY stability, impermanent loss risk, and overall risk score.",
        inputSchema: {
            type: "object",
            properties: {
                vault: {
                    type: "string",
                    description: "Vault name or identifier (e.g. aave-v3, morpho, yearn)",
                },
                chain: {
                    type: "string",
    ...
    {
        name: "compare_vaults",
        description: "Compare risk metrics between multiple vaults for the same asset on a chain.",
        inputSchema: {
            type: "object",
            properties: {
                chain: {
    ...
    },
    {
        ...
        description: "Find the safest vaults for a given asset and chain. Prioritizes high TVL, audited protocols, and stable APY.",
        inputSchema: {
            type: "object",
            properties: {
                chain: {
...
async function searchPools(asset, chain) {
    const pools = await fetchPools();
    const assetUpper = asset.toUpperCase();
    const chainLower = chain.toLowerCase();
    return pools.filter((p) => {
        if (chainLower && p.chain?.toLowerCase() !== chainLower)
            return false;
        if (assetUpper) {
            if (!p.symbol.toUpperCase().includes(assetUpper) &&
...
async function handleSearchVaults(args) {
...
async function handleGetVaultRiskBreakdown(args) {
...
async function handleCompareVaults(args) {
    const chain = String(args.chain ?? "");
    const asset = String(args.asset ?? "").toUpperCase();
    const pools = await searchPools(asset, chain);
    pools.sort((a, b) => (b.tvlUsd || 0) - (a.tvlUsd || 0));
...
async function handleFindSafestVaults(args) {
    const chain = String(args.chain ?? "").toLowerCase();
    const asset = String(args.asset ?? "").toUpperCase();
...
export async function callPhilidorTool(name, args) {
    switch (name) {
        case "search_vaults":
