...
export const COINGECKO_TOOLS = [
    {
        name: "execute",
        description: "Execute a CoinGecko API endpoint. Use endpoint='/search/trending' for trending tokens, '/simple/price' for price lookups, '/coins/markets' for market data.",
        inputSchema: {
            type: "object",
            properties: {
                endpoint: {
...
async function handleExecute(args) {
    ...
    try {
        ...
        if (args.vs_currency)
            params.set("vs_currency", String(args.vs_currency));
        if (args.ids)
            params.set("ids", String(args.ids));
...
export async function callCoingeckoTool(name, args) {
    if (name === "execute")
